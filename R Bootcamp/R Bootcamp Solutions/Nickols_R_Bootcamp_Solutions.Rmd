...
The limit is $0$.  As shown [here](https://www.cs.umd.edu/class/spring2016/cmsc351-0101/harmonic.pdf), $\sum_{i=1}^n\frac{1}{i}\leq \log_2(n+1)$.  Also, by L'Hospital's rule, $\lim_{x\rightarrow\infty}\frac{\log_2(x+1)}{x}=\lim_{x\rightarrow\infty}\frac{1}{(x+1)\ln(2)}\rightarrow0$.  Thus, $$0<\frac{1}{n}\sum_{i=1}^n\frac{1}{i}\leq \frac{\log_2(n+1)}{n}\rightarrow 0$$
...
```{r, warning=FALSE}
over_20 <- countries[countries$mad_gdppc > 20000,]
ggplot(over_20, aes(x = wdi_expedu, y = spi_ospi)) +
  geom_point() +
...
person_1 <- c(1:s, (s+1):(s+n_1)) # s intersecting, n_1 unique
person_2 <- c(1:s, (s+n_1+1):(s+n_1+n_2))
person_3 <- c(1:s, (s+n_1+n_2+1):(s+n_1+n_2+n_3))

# Find the length of the vector of intersecting samples (number of intersections)
make_draw <- function() { 
  return (length(intersect(sample(person_1, m), 
                           intersect(sample(person_2, m), 
                                     sample(person_3, m)))))
}

n_draw <- 10000
draws <- replicate(n_draw, make_draw()) # Run the function n_draw times

df <- data.frame("draws" = c(draws, rchyper(n_draw, 
                                            s, 
                                            c(n_1, n_2, n_3), 
                                            c(m, m, m), 
                                            verbose = F)), 
                 "type" = c(rep(c("simulated", "chyper"), each = n_draw)))

# Plot a histogram
ggplot(df, aes(x = draws, fill = type)) + 
  geom_histogram(alpha = 0.5, position = "identity", bins = 30) + 
  theme_bw() + 
  xlab("Draws")

# Top row is the proportion of draws
# Bottom row is the true PMF
print(round(rbind(table(draws) / length(draws), 
                  dchyper(0:max(draws), 
                          s, 
                          c(n_1, n_2, n_3), 
                          c(m, m, m), 
