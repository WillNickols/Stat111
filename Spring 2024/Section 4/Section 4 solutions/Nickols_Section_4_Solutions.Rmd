...
Our log likelihood only has a non-zero gradient with respect to $\beta$ and jumps to $-\infty$ once the indicators fail. Probably the best thing to do here is to use differentiable functions to very severely penalize $\alpha$ and $\beta$ that get close to the min and max. The following gives a case of $\alpha=10$ and $\beta=5$ with $n=100$ uniform random variables. It uses the `optim` function in R to estimate $\alpha$ and $\beta$ from starting guesses of $(20,100)$ by maximizing the likelihood function.
