...
$$\mathcal{I}_n(\mu)=\textrm{Var}(s(\mu;\vec{Y}))=\frac{1}{\sigma^4}\sum_{i=1}^n\textrm{Var}(Y_i)=\frac{n}{\sigma^2}$$
...
$$L(\alpha,\beta;\vec{y})=\prod_{i=1}^n\frac{I(\alpha-\beta/2\leq y_i\leq \alpha+\beta/2)}{\beta}=\frac{I(\alpha-\beta/2\leq y_{(1)})I(y_{(n)}\leq \alpha+\beta/2)}{\beta^n}$$
